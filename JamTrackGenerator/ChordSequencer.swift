...
struct ChordSequencer {
    ...
    init(form: String, key: String) {
        self.form = form
        self.key = Key(key)
    }
    
    func calcChordProgression() -> [Measure] {
        let oneChord = Chord(name: key.root + "7", duration: .whole)
        let fourChord = Chord(name: key.fourth + "7", duration: .whole)
        let fiveChord = Chord(name: key.fifth + "7", duration: .whole)
