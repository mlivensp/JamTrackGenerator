...
//    static let pattern05: Pattern = Pattern(chorus: pattern05Chorus())
    ...
    init(specification: JamTrackSpecification) {
        self.specification = specification
    }
    
    var pattern05: Pattern {
        Pattern(chorus: pattern05Chorus(specification: specification))
    }
    
    func pattern05Chorus(specification: JamTrackSpecification) -> [DrumDescriptor] {
        var events: [DrumDescriptor] = []
        var pulseAtMeasureStart: UInt32 = 0
        var counter = 0
        // bars 1-4
        events.append(contentsOf: createPattern0Measure(specification: specification, pulseAtMeasureStart: pulseAtMeasureStart))
    ...
    fileprivate func createPattern0Measure(specification: JamTrackSpecification, pulseAtMeasureStart: UInt32) -> [DrumDescriptor] {
        let firstEighth: NoteDuration
        let secondEighth: NoteDuration
        
        if specification.feel == .straight {
