...
struct HarmonicPatternEditView: View {
    ...
    @Binding var harmonicPattern: HarmonicPattern
    
    init(harmonicPattern: Binding<HarmonicPattern>, navPath: Binding<NavigationPath>) {
        self._harmonicPattern = harmonicPattern
        self._navPath = navPath
        self.viewModel = .init(harmonicPattern: harmonicPattern.wrappedValue)
    }
    
    var body: some View {
        Form {
            Section(header: Text("Harmonic Pattern Info")) {
                VStack {
...
//            }
        ...
        .navigationTitle(viewModel.name.isEmpty ? "New Harmonic Pattern" : viewModel.name)
#if os(iOS)
...
#Preview {
    ...
    let container = try! ModelContainer(for: HarmonicPattern.self, configurations: config)
    
    let harmonicPattern = HarmonicPattern(name: "Preview Harmonic Pattern")
    container.mainContext.insert(harmonicPattern)
    
    return HarmonicPatternEditView(harmonicPattern: .constant(harmonicPattern), navPath: .constant(NavigationPath()))
